...
int main() {
    ...
    DMatrix QN = 1e3 * eye<double>(Jn.getDim());
...
//    bool constrainVDot   = false;
    bool constrainTau    = true;
    bool constrainTauDot = true;

    // Constraints' values
//    DVector vLimit      = 2 * ones<double>(6);   // Maximum/minimun velocity allowed
//    DVector vDotLimit   = 0.3 * ones<double>(6); // Maximum/minimun acceleration allowed
    DVector tauDotLimit = 1e3 * ones<double>(6); // Maximum/minimun effort rate allowe
    DVector tauLimit    = 120 * ones<double>(6); // Maximum/minimun effort allowed
...
//    if (constrainV) {
//        ocp.subjectTo(-vLimit(0) <= v(0) <= vLimit(0));
//        ocp.subjectTo(-vLimit(1) <= v(1) <= vLimit(1));
//        ocp.subjectTo(-vLimit(2) <= v(2) <= vLimit(2));
//        ocp.subjectTo(-vLimit(4) <= v(4) <= vLimit(4));
//        ocp.subjectTo(-vLimit(5) <= v(5) <= vLimit(5));
//    }
//
//    //	VELOCITY DERIVATIVE CONSTRAINTS
//
//    if (constrainVDot) {
//        ocp.subjectTo(-vDotLimit(0) <= vDot(0) <= vDotLimit(0));
//        ocp.subjectTo(-vDotLimit(1) <= vDot(1) <= vDotLimit(1));
//        ocp.subjectTo(-vDotLimit(2) <= vDot(2) <= vDotLimit(2));
//        ocp.subjectTo(-vDotLimit(4) <= vDot(4) <= vDotLimit(4));
//        ocp.subjectTo(-vDotLimit(5) <= vDot(5) <= vDotLimit(5));
//    }
